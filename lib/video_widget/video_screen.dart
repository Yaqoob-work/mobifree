...
class _VideoScreenState extends State<VideoScreen> with WidgetsBindingObserver {
  ...
  Future<void> _initializeVLCController() async {
    ...
    String modifiedUrl =
        '${widget.videoUrl}?network-caching=2000&live-caching=1000&rtsp-tcp';
  ...
  //         '${updatedUrl}?network-caching=2000&live-caching=1000&rtsp-tcp';
  ...
  // Future<Map<String, dynamic>> fetchLiveFeaturedTVByIdOrName({
  //   required String channelId,
  //   required String channelName,
  // }) async {
  //   const String apiKey = 'vLQTuPZUxktl5mVW';

  //   try {
  //     // Try fetching by ID
  //     if (channelId.isNotEmpty) {
  //       final idResponse = await https.get(
  //         Uri.parse('https://acomtv.com/android/searchContent/$channelId/0'),
  //         headers: {'x-api-key': apiKey},
  //       );

  //       if (idResponse.statusCode == 200) {
  //         final Map<String, dynamic> idData = jsonDecode(idResponse.body);
  //         if (idData['url'] != null && idData['url'].isNotEmpty) {
  //           return {
  //             'url': idData['url'],
  //             'stream_type': idData['stream_type'] ?? '',
  //           };
  //         }
  //       }
  //     }
  ...
  //     final nameResponse = await https.get(
  //       Uri.parse('https://acomtv.com/android/searchContent/$channelName/0'),
  //       headers: {'x-api-key': apiKey},
  //     );

  //     if (nameResponse.statusCode == 200) {
  //       final Map<String, dynamic> nameData = jsonDecode(nameResponse.body);
  //       if (nameData['url'] != null && nameData['url'].isNotEmpty) {
  //         return {
  //           'url': nameData['url'],
  //           'stream_type': nameData['stream_type'] ?? '',
  //         };
  //       } else {
  //         throw Exception('Invalid data received: URL is missing or empty');
  //       }
  //     } else {
  //       throw Exception(
  //           'Failed to fetch data by name: ${nameResponse.statusCode} - ${nameResponse.reasonPhrase}');
  //     }
  //   } catch (e) {
  //     print('Error fetching live featured TV by ID or name: $e');
  //     throw Exception('Error fetching live featured TV by ID or name: $e');
  //   }
  // }
  ...
  try {
    ...
    // Fallback to fetching by trimmed Name
    final trimmedName = channelName.trim(); // Trim spaces from both ends
    final nameResponse = await https.get(
      Uri.parse('https://acomtv.com/android/searchContent/$trimmedName/0'),
      headers: {'x-api-key': apiKey},
    );
...
//     _connectivityCheckTimer = Timer.periodic(Duration(seconds: 5), (_) async {
//       bool isConnectedNow = await _checkNetworkConnectivity();
...
//           _resumeVideo();
//         }
//       } else if (!isConnectedNow && _isConnected) {
//         print("Disconnected from the network");
//         _isConnected = false;
//         if (_controller != null && _controller!.value.isPlaying) {
//           _wasPlayingBeforeDisconnection = true;
//           _controller?.pause();
//         }
//       }
//     });
...
//     if (_controller != null && _controller!.value.isInitialized) {
//       await _controller!.seekTo(_lastKnownPosition); // Seek to last position
//       await _controller!.play(); // Resume playback
//       _wasPlayingBeforeDisconnection = false; // Reset flag
//     }
//   }

//   // void _initializeVLCController() {
//   //   String modifiedUrl =
//   //       '${widget.videoUrl}?network-caching=1000&live-caching=500&rtsp-tcp';
...
//   //   _controller!.addListener(() {
//   //     if (_controller!.value.isPlaying) {
//   //       _lastKnownPosition = _controller!.value.position; // Save position
//   //     }
...
//         _isVideoInitialized = true;
//       });
//       Timer(Duration(seconds: widget.isVOD ? 9 : 5), () {
//         setState(() {
//           _loadingVisible = false;
//         });
//       });
...
//       if (
//           // selectedChannel is NewsItemModel &&
...
//       if (
//           // widget.isVOD
//           widget.source == 'isContentScreenFromDetailsPageChannelLIst') {
//         final playLink = await fetchMoviePlayLink(contentId);
//         if (playLink['url'] != null && playLink['url']!.isNotEmpty) {
//           updatedUrl = playLink['url']!;
//           if (playLink['type'] == 'Youtube' ||
...
//       if (_controller != null && _controller!.value.isInitialized) {
//         // String modifiedUrl = '${updatedUrl}?network-caching=3000';
//         String modifiedUrl =
//             '${updatedUrl}?network-caching=1000&live-caching=500&rtsp-tcp';
//         await _controller!.setMediaFromNetwork(modifiedUrl);
//         await _controller!.play();
...
//           // _loadingVisible = false; // Hide loading indicator
//         });
//         Timer(Duration(seconds: widget.isVOD ? 8 : 5), () {
//           setState(() {
//             _loadingVisible = false;
//           });
//         });
