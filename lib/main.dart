...
void callbackDispatcher() {
  Workmanager().executeTask((task, inputData) async {
    // Fetch settings and entertainment in the background
    final apiService = ApiService();
    await apiService.fetchSettings();
    await apiService.fetchEntertainment();
    
    return Future.value(true); // Signal completion of the task
  });
}
...
void main() {
  ...
  Workmanager().initialize(
    callbackDispatcher, // The callbackDispatcher function
    isInDebugMode: true, // Enable debug mode for testing
  );

  // Register a periodic task to run every 1 hour (min interval is 15 minutes)
  Workmanager().registerPeriodicTask(
    "1", // Unique identifier for the task
    "updateCache", // Name of the task
    frequency: Duration(hours: 1), // Set the interval time here
  );
