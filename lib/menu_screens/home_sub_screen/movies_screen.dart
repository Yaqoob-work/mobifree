...
//         if (itemFocusNodes.containsKey(firstItemId)) {
//           final focusNode = itemFocusNodes[firstItemId]!;
//           context.read<FocusProvider>().setFirstManageMoviesFocusNode(focusNode);
//           print("✅ MoviesScreen: First item focus node registered: $firstItemId");
//         } else {
//           print("⚠️ MoviesScreen: First item NOT registered!");
//         }
//       }
//     });
...
//       // Convert to NewsItemModel list
//       var cachedItems = cachedData.map((item) => NewsItemModel.fromJson(item)).toList();
      
//       // Sort by index
//       cachedItems.sort((a, b) {
//         int aIndex = int.tryParse(a.index) ?? 0;
//         int bIndex = int.tryParse(b.index) ?? 0;
//         return aIndex.compareTo(bIndex);
//       });

//       setState(() {
//         _watchlistItems = cachedItems;
//         _isLoading = false;
//       });
      
//       // Initialize focus nodes for cached items
//       _initializeItemFocusNodes();
//     }
//   } catch (e) {
//     print('Error loading cached watchlist data: $e');
//   }
...
// //                   status: item['status'] ?? '',
// //                   videoId: item['video_id'] ?? '',
// //                   index: item['index']?.toString() ?? '', // Make sure to include index
// //                 ))
// //             .toList();

// //         // Sort by index (convert to int for proper numeric sorting)
// //         newsItems.sort((a, b) {
// //           int aIndex = int.tryParse(a.index) ?? 0;
// //           int bIndex = int.tryParse(b.index) ?? 0;
// //           return aIndex.compareTo(bIndex);
...
//                   status: item['status'] ?? '',
//                   videoId: item['video_id'] ?? '',
//                   index: item['index']?.toString() ?? '0', // Default '0' if null
//                 ))
//             .toList();

//         // Sort by index (Ascending: Chhote index pehle)
//         newsItems.sort((a, b) {
//           int aIndex = int.tryParse(a.index) ?? 0;
//           int bIndex = int.tryParse(b.index) ?? 0;
//           return aIndex.compareTo(bIndex);
...
//   // Similar caching implementation as MusicScreen
//   Future<void> _loadCachedDataAndFetchWatchlist() async {
//     setState(() {
//       _isLoading = true;
//       _errorMessage = '';
...
//       // Step 2: Fetch new data in background
//       await _fetchWatchlistInBackground();
//     } catch (e) {
//       setState(() {
//         _errorMessage = 'Failed to load watchlist data';
...
//       final newWatchlistItems = await _fetchWatchlistFromApi();
//       final prefs = await SharedPreferences.getInstance();
//       final currentWatchlistJson = json.encode(_watchlistItems.map((e) => e.toJson()).toList());
//       final newWatchlistJson = json.encode(newWatchlistItems.map((e) => e.toJson()).toList());

//       // Only update if data has changed
//       if (currentWatchlistJson != newWatchlistJson) {
//         await prefs.setString('watchlist_data', newWatchlistJson);
...
//   void _initializeItemFocusNodes() {
//     for (var item in _watchlistItems) {
//       itemFocusNodes.putIfAbsent(item.id, () => FocusNode()
//         ..addListener(() {
//           if (itemFocusNodes[item.id]!.hasFocus) {
//             _scrollToFocusedItem(item.id);
//           }
